"""Agent 4 - Natural language report writer."""
...
def generate_report(insights: dict, audit_log: list[str], client: OpenAI) -> str:
    """
    Use an LLM to write a full natural-language data analysis report.
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "You are DataForge, a professional AI data analyst. "
                    "Write clear, accurate, insightful reports."
                ),
            },
...
def generate_report_fallback(insights: dict, audit_log: list[str]) -> str:
    ...
    lines.append(
        f"The dataset contains **{shape.get('rows', '?')} rows** and "
        f"**{shape.get('cols', '?')} columns**. "
        "DataForge has autonomously cleaned, analysed and visualised this data. "
        f"A total of **{len(cols)} features** were examined, "
        f"with **{len(numeric)} numeric** and **{len(categorical)} categorical** "
        "columns identified.\n"
    )
    ...
    lines.append(
        f"- **Numeric columns:** {', '.join(numeric.keys()) if numeric else 'None'}"
    )
    lines.append(
        f"- **Categorical columns:** "
        f"{', '.join(categorical.keys()) if categorical else 'None'}\n"
    )
    ...
    if correlations:
        for pair in correlations:
            ...
            lines.append(
                f"- **{pair['col_a']}** and **{pair['col_b']}**: {strength} "
                f"correlation (r = {pair['correlation']})"
            )
        lines.append("")
    else:
        lines.append(
            "No strong correlations (r > 0.5) were detected between numeric columns.\n"
        )
    ...
    recs = [
        ...
        (
            "Investigate any flagged outliers; they may represent genuine anomalies "
            "or data entry errors."
        ),
    ]
    if correlations:
        recs.append(
            f"Explore the relationship between '{correlations[0]['col_a']}' and "
            f"'{correlations[0]['col_b']}' further; strong correlation may indicate "
            "a causal or confounding relationship."
        )
    if skewed:
        recs.append(
            "Consider applying log transformation to skewed columns "
            f"({', '.join(skewed.keys())}) before using in predictive models."
        )
